...
import { NavigationPrototype } from './navigation-prototype.js';
...
export class RouteCrafterApp {
    ...
    init() {
        // Suprimir aviso de depreciação do Leaflet Draw (usa _flat em vez de L.LineUtil.isFlat)
        const originalWarn = console.warn;
        console.warn = function(...args) {
            if (args.length > 0 && typeof args[0] === 'string' && args[0].includes('Deprecated use of _flat')) {
                return; // Suprimir esse aviso específico
            }
        ...
        // Initialize in-browser navigation prototype
        this.navigationPrototype = new NavigationPrototype(this.mapManager, this.routingManager);
    ...
    const coverageThresholdInput = document.getElementById('coverageThreshold');
        ...
        const trimModeButton = document.getElementById('trimModeButton');
        const trimSaveButton = document.getElementById('trimSaveButton');
        const trimCancelButton = document.getElementById('trimCancelButton');
        const updateTrimButtons = (active) => {
            if (!trimModeButton) return;
            if (active) {
                trimModeButton.classList.add('active');
                trimModeButton.textContent = 'Trim Mode (ON)';
                if (trimSaveButton) trimSaveButton.style.display = 'inline-block';
                if (trimCancelButton) trimCancelButton.style.display = 'inline-block';
            } else {
                trimModeButton.classList.remove('active');
                trimModeButton.textContent = 'Trim Mode';
                if (trimSaveButton) trimSaveButton.style.display = 'none';
                if (trimCancelButton) trimCancelButton.style.display = 'none';
            }
        };

        if (trimModeButton) {
            trimModeButton.addEventListener('click', () => {
                if (!this.roadProcessor) return;
                if (typeof this.roadProcessor.isTrimModeActive === 'function' && this.roadProcessor.isTrimModeActive()) {
                    this.roadProcessor.cancelTrimMode();
                    updateTrimButtons(false);
                } else {
                    const enabled = this.roadProcessor.enableTrimMode && this.roadProcessor.enableTrimMode();
                    if (enabled) {
                        updateTrimButtons(true);
                    }
                }
            });
        }
        ...
        if (trimSaveButton) {
            trimSaveButton.addEventListener('click', () => {
                if (!this.roadProcessor || !this.roadProcessor.applyTrimChanges) return;
                this.roadProcessor.applyTrimChanges();
                updateTrimButtons(false);
            });
        }
        ...
        if (trimCancelButton) {
            trimCancelButton.addEventListener('click', () => {
                if (!this.roadProcessor || !this.roadProcessor.cancelTrimMode) return;
                this.roadProcessor.cancelTrimMode();
                updateTrimButtons(false);
            });
        }
        ...
        // Default to Draw Area on initial load for quicker manual outlining
        const searchRulesSelect = document.getElementById('searchRules');
        if (searchRulesSelect) {
            searchRulesSelect.value = 'draw_area';
            searchRulesSelect.dispatchEvent(new Event('change'));
            if (this.mapManager && typeof this.mapManager.enableDrawControl === 'function') {
                this.mapManager.enableDrawControl();
            }
        }
    }
    ...
    button.innerHTML = 'Generating Route <span class="spinner"></span>';
        ...
        try {
            ...
            // Se o Trim Mode estiver ativo, garantir que continue funcionando após gerar a rota
            if (this.roadProcessor && this.roadProcessor.isTrimModeActive && this.roadProcessor.isTrimModeActive()) {
                // Reativar o Trim Mode para garantir que os event handlers estejam corretos
                const wasActive = true;
                this.roadProcessor.disableTrimModeInternal({ resetStyles: false });
                this.roadProcessor.enableTrimMode();
                console.log('Trim Mode reativado após gerar rota');
            }
        } catch (error) {
    ...
    clearAllSelectionsWithoutResettingDropdown() {
        ...
        try {
            document.dispatchEvent(new CustomEvent('routecrafter:route-updated', { detail: { points: [] } }));
        } catch (err) {
            console.warn('Failed to dispatch route updated event:', err);
        }
        
        if (this.navigationPrototype) {
            this.navigationPrototype.reset();
        }
