...
export class AreaManager {
    ...
    handleDrawCreated(event) {
        ...
        if (type === 'polygon' || type === 'rectangle') {
            ...
            layer._clickHandler = (e) => {
                ...
                if (drawControl && drawControl._toolbars && drawControl._toolbars.edit && drawControl._toolbars.edit._activeMode) {
    ...
    previewDrawnAreaWithBuffer(drawnLayer) {
        ...
        const coordinates = drawnLayer.getLatLngs()[0];
            ...
            // Converter coordenadas e garantir que o polígono está fechado (primeiro = último)
            const coords = coordinates.map(coord => [coord.lng, coord.lat]);
            // Verificar se primeiro e último são diferentes, se sim, fechar o polígono
    ...
    disablePolygonInteraction() {
        // Desabilitar interação com todos os polígonos desenhados
        this.mapManager.getDrawnItems().eachLayer((layer) => {
            if (layer instanceof L.Polygon || layer instanceof L.Rectangle) {
                // Remover temporariamente o handler de clique
                if (layer._clickHandler) {
                    layer.off('click', layer._clickHandler);
                }
                // Tornar o polígono não interativo e reduzir z-index
                ...
                // Reduzir z-index para que fique abaixo das vias
                if (layer._path) {
                    layer._path.style.pointerEvents = 'none';
                    layer._path.style.zIndex = '100';
                }
            }
        });
        
        // Desabilitar interação com polígonos destacados
        this.highlightedPolygons.forEach((layer) => {
            layer.options.interactive = false;
            if (layer.setStyle) {
                layer.setStyle({ 
    ...
    enablePolygonInteraction() {
        // Reabilitar interação com todos os polígonos desenhados
        this.mapManager.getDrawnItems().eachLayer((layer) => {
            if (layer instanceof L.Polygon || layer instanceof L.Rectangle) {
                ...
                if (layer._path) {
                    ...
                    layer._path.style.zIndex = '';
                }
                // Re-adicionar handler de clique se foi removido
                if (layer._clickHandler && !layer.listens('click')) {
                    layer.on('click', layer._clickHandler);
                } else if (!layer._clickHandler) {
        ...
        // Reabilitar interação com polígonos destacados
        this.highlightedPolygons.forEach((layer) => {
            layer.options.interactive = true;
            if (layer.setStyle) {
                layer.setStyle({ 
