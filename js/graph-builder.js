...
`;
    ...
    /**
     * Detecta se uma feature é uma rotatória/praça circular pela geometria
     */
    isCircularRoad(feature) {
        const coords = feature.geometry.coordinates;
        if (!coords || coords.length < 5) {
            return false; // Muito poucos pontos para ser circular
        }
        
        // Calcular o comprimento total e verificar se forma um círculo
        let totalLength = 0;
        const distances = [];
        ...
        // Verificar se o primeiro e último ponto são próximos (forma um loop)
        const firstPoint = turf.point(coords[0]);
        const lastPoint = turf.point(coords[coords.length - 1]);
        const loopDistance = turf.distance(firstPoint, lastPoint, { units: 'meters' });
        
        // Se o primeiro e último ponto estão próximos (menos de 10% do comprimento total)
        // e o comprimento total é razoável (entre 50m e 2000m), pode ser uma rotatória
        const isLoop = loopDistance < (totalLength * 0.1);
        const reasonableSize = totalLength >= 50 && totalLength <= 2000;
        
        if (isLoop && reasonableSize) {
            ...
            // Verificar se os pontos estão aproximadamente à mesma distância do centro
            const distancesFromCenter = coords.map(coord => {
                const p = turf.point(coord);
                return turf.distance(p, center, { units: 'meters' });
            });
            ...
            // Se o desvio padrão é pequeno (menos de 20% da média), é provavelmente circular
            const isCircular = stdDev < (avgDistance * 0.2);
    ...
    /**
     * Otimiza rotatórias para evitar múltiplas voltas desnecessárias
     * Marca apenas uma passagem como obrigatória, tornando as demais opcionais
     */
    optimizeRoundabouts(roadFeatures) {
        const roundaboutFeatures = [];
        
        // Identificar todas as rotatórias (por propriedade ou geometria)
        roadFeatures.forEach((feature, index) => {
            const props = feature.properties || {};
            const isMarkedRoundabout = props.junction === 'roundabout' || 
                                      (props.highway && props.highway.includes('roundabout'));
            const isCircular = this.isCircularRoad(feature);
            
            if (isMarkedRoundabout || isCircular) {
                roundaboutFeatures.push({ feature, index, isMarked: isMarkedRoundabout });
            }
        });
        
        if (roundaboutFeatures.length === 0) {
            return roadFeatures; // Sem rotatórias, retornar original
        }
        
        console.log(`Detectadas ${roundaboutFeatures.length} rotatória(s), otimizando...`);
        
        // Para cada rotatória, marcar apenas uma volta como obrigatória
        roundaboutFeatures.forEach(({ feature, index, isMarked }) => {
            const coords = feature.geometry.coordinates;
            if (!coords || coords.length < 3) {
                return; // Rotatória muito pequena, ignorar
            }
            
            // Calcular o comprimento total da rotatória
            let totalLength = 0;
            for (let i = 0; i < coords.length - 1; i++) {
                const p1 = turf.point(coords[i]);
                const p2 = turf.point(coords[i + 1]);
                totalLength += turf.distance(p1, p2, { units: 'meters' });
            }
            
            // Para rotatórias grandes (mais de 150m), marcar apenas uma parte como obrigatória
            // Para rotatórias pequenas, marcar toda como obrigatória mas aumentar o peso
            if (totalLength > 150) {
                // Para rotatórias grandes, marcar apenas ~25% dos segmentos como obrigatórios
                const requiredSegments = Math.max(1, Math.floor((coords.length - 1) * 0.25));
                feature.properties._roundaboutOptimized = true;
                feature.properties._roundaboutRequiredSegments = requiredSegments;
                console.log(`${isMarked ? 'Rotatória marcada' : 'Praça circular detectada'} (${totalLength.toFixed(1)}m), marcando ${requiredSegments} de ${coords.length - 1} segmentos como obrigatórios`);
            } else {
                // Para rotatórias pequenas, marcar toda como obrigatória mas aumentar peso
                feature.properties._roundaboutOptimized = true;
                feature.properties._roundaboutRequiredSegments = coords.length - 1;
                console.log(`${isMarked ? 'Rotatória pequena' : 'Rotatória circular pequena'} detectada (${totalLength.toFixed(1)}m)`);
            }
        });
    ...
    buildChinesePostmanGraph(roadFeatures, coordinateToNodeIdMap, nodeIdToCoordinateMap, exportFormat = 'windy_rural_benavent') {
        ...
        // Otimizar rotatórias antes de processar
        roadFeatures = this.optimizeRoundabouts(roadFeatures);
        ...
        roadFeatures.forEach((feature, featureIndex) => {
            ...
            let isRouteRequired = properties.isRouteRequired !== undefined ? properties.isRouteRequired : true;
            
            // Aplicar otimização de rotatórias
            const isRoundabout = properties.junction === 'roundabout' || 
                                (properties.highway && properties.highway.includes('roundabout'));
            const isRoundaboutOptimized = properties._roundaboutOptimized === true;
            ...
            for (let i = 0; i < coordinates.length - 1; i++) {
                ...
                let distance = turf.distance(sourcePoint, targetPoint, { units: 'meters' });
                
                // Skip zero-length edges
                if (distance === 0) continue;
                
                // Para rotatórias otimizadas, marcar apenas uma parte como obrigatória
                let segmentIsRequired = isRouteRequired;
                if (isRoundabout && isRoundaboutOptimized) {
                    const requiredSegments = properties._roundaboutRequiredSegments || (coordinates.length - 1);
                    // Marcar apenas os primeiros segmentos como obrigatórios
                    segmentIsRequired = i < requiredSegments;
                    // Aumentar o peso dos segmentos opcionais para desencorajar múltiplas passagens
                    if (!segmentIsRequired) {
                        distance = distance * 20; // Aumentar custo em 20x para segmentos opcionais
                    } else if (requiredSegments < (coordinates.length - 1)) {
                        // Mesmo para segmentos obrigatórios em rotatórias grandes, aumentar ligeiramente o peso
                        // para desencorajar múltiplas voltas completas
                ...
                if (exportFormat === 'undirected') {
                    ...
                    edges.push({
                        ...
                        isCovered: isCovered,
                        isRouteRequired: segmentIsRequired
                    });
