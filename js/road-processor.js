...
export class RoadProcessor {
    constructor(mapManager, areaManager, coverageManager) {
        ...
        this.trimNodeLayer = null;
        this.trimNodeStats = new Map();
    }
    ...
    formatCoordinateKey(coord) {
        if (!coord || coord.length < 2) return '';
        // coord order is [lon, lat]
        return `${coord[0].toFixed(7)},${coord[1].toFixed(7)}`;
    }

    segmentRoadNetwork(originalFeatures) {
        this.trimNodeStats = new Map();
        const nodeStats = this.trimNodeStats;

        // First pass: count how many times each coordinate appears across all features
        originalFeatures.forEach((feature) => {
            ...
            coords.forEach((coord) => {
                const key = this.formatCoordinateKey(coord);
        ...
        originalFeatures.forEach((feature) => {
            if (!feature || !feature.geometry || feature.geometry.type !== 'LineString') {
                segmentedFeatures.push(feature);
                return;
            }
            ...
            for (let i = 1; i < coords.length; i++) {
                ...
                const key = this.formatCoordinateKey(coord);
                const nodeInfo = nodeStats.get(key);
                const isSplitNode = nodeInfo && nodeInfo.count > 1 && i < coords.length - 1;

                if (isSplitNode) {
                    const newFeature = this.createSegmentFeature(feature, currentSegmentCoords);
                    segmentedFeatures.push(newFeature);
                    currentSegmentCoords = [coord];
                }
            }

            if (currentSegmentCoords.length >= 2) {
                segmentedFeatures.push(this.createSegmentFeature(feature, currentSegmentCoords));
            }
        });

        segmentedFeatures.forEach((feature) => {
            if (!feature.properties) {
                feature.properties = {};
            }
            feature.properties.rcSegmentId = this.segmentIdCounter++;
        });

        return segmentedFeatures;
    }

    createSegmentFeature(sourceFeature, coordinates) {
        const clonedCoords = coordinates.map(coord => [...coord]);
        return {
            type: 'Feature',
            geometry: {
                type: 'LineString',
    ...
    fetchRoadsInArea(createCoordinateMappingsCallback) {
        ...
        if (this.trimModeActive) {
            this.disableTrimModeInternal({ resetStyles: false });
        } else {
            this.hideTrimNodeMarkers();
        }
        ...
        try {
            ...
            fetch(overpassEndpoint, {
                ...
                // Reset segment counter and split roads at intersections
                this.segmentIdCounter = 1;
                roadFeatures = this.segmentRoadNetwork(roadFeatures);
                ...
                // Adicionar ao mapa com z-index alto para ficar acima dos polígonos
                this.geoJsonLayer.addTo(this.mapManager.getMap());
                // Ajustar z-index para garantir que fique acima dos polígonos
                if (this.geoJsonLayer.eachLayer) {
    ...
    isTrimModeActive() {
        return this.trimModeActive;
    }

    enableTrimMode() {
        if (!this.geoJsonLayer) {
            alert('Nenhum conjunto de vias disponível. Clique em "Fetch Roads" antes de usar o Trim Mode.');
            return false;
        }
        if (this.trimModeActive) {
            return true;
        ...
        this.trimModeActive = true;

        this.geoJsonLayer.eachLayer((layer) => {
            if (!layer || !layer.feature || layer.feature.geometry.type !== 'LineString') {
                return;
            }

            const isRequired = layer.feature.properties && layer.feature.properties.isRouteRequired !== false;
            layer.__trimOriginalRequired = isRequired;
            layer.__trimState = isRequired;
            layer.__trimClickHandler = (event) => this.handleTrimLayerClick(event);
            layer.on('click', layer.__trimClickHandler);
            // Remover tooltip se existir
            if (layer.getTooltip && layer.getTooltip()) {
                layer.unbindTooltip();
            }
            // Desabilitar popup durante Trim Mode
            if (layer.getPopup && layer.getPopup()) {
                layer.unbindPopup();
            }
            // Garantir que a via seja clicável e fique acima dos polígonos e da solução
            layer.options.interactive = true;
            if (layer._path) {
                layer._path.style.pointerEvents = 'auto';
                layer._path.style.zIndex = '2000'; // Acima da solução (z-index 1000)
            }
            // Garantir que o layer inteiro fique acima
            if (layer.bringToFront) {
                layer.bringToFront();
            }
            this.applyTrimPreviewVisual(layer);
        });

        this.showTrimNodeMarkers();
        
        // Garantir que o geoJsonLayer fique acima da camada de solução
        if (this.geoJsonLayer && this.geoJsonLayer.bringToFront) {
            this.geoJsonLayer.bringToFront();
        }
        
        // Desabilitar interação com polígonos durante Trim Mode
        if (this.areaManager && typeof this.areaManager.disablePolygonInteraction === 'function') {
            this.areaManager.disablePolygonInteraction();
        }
    ...
    applyTrimChanges() {
        if (!this.trimModeActive) {
            alert('Trim Mode não está ativo.');
            return;
        }

        let updatedCount = 0;
        this.geoJsonLayer.eachLayer((layer) => {
            if (layer.__trimState === undefined) {
                return;
            }

            if (layer.__trimState !== layer.__trimOriginalRequired) {
                if (!layer.feature.properties) {
                    layer.feature.properties = {};
                }
                layer.feature.properties.isRouteRequired = layer.__trimState;
                layer.__trimOriginalRequired = layer.__trimState;
                updatedCount++;
            }
        });

        this.disableTrimModeInternal({ resetStyles: true });

        if (updatedCount > 0) {
            alert('Trim salvo. Gere a rota novamente para aplicar as alterações.');
        } else {
            alert('Nenhuma alteração foi registrada.');
        }
    }
    ...
    cancelTrimMode() {
        ...
        this.geoJsonLayer.eachLayer((layer) => {
            ...
            layer.__trimState = layer.__trimOriginalRequired;
        });

        this.disableTrimModeInternal({ resetStyles: true });
    }

    handleTrimLayerClick(event) {
        if (!this.trimModeActive || !event || !event.target) {
            return;
        }
        ...
        const wasRequired = layer.__trimState;
        layer.__trimState = !layer.__trimState;
        this.applyTrimPreviewVisual(layer);
        
        // Se o segmento foi removido (de required para false), verificar desconexões
        if (wasRequired && !layer.__trimState) {
            this.removeDisconnectedSegments();
        }
    }

    applyTrimPreviewVisual(layer) {
        if (!layer || layer.__trimState === undefined) {
            return;
        }
        const style = layer.__trimState ? this.trimPreviewStyles.required : this.trimPreviewStyles.removed;
        layer.setStyle(style);
    }

    removeDisconnectedSegments() {
        // Construir grafo de conectividade: mapear cada nó para os segmentos que o conectam
        const nodeToSegments = new Map(); // nodeKey -> Set de layers
        const segmentToNodes = new Map(); // layer -> [nodeKey1, nodeKey2]
        const allActiveSegments = [];
        
        this.geoJsonLayer.eachLayer((layer) => {
            if (!layer || !layer.feature || layer.feature.geometry.type !== 'LineString') {
                return;
            }
            // Apenas considerar segmentos que estão marcados como required (ainda ativos)
            if (layer.__trimState === false) {
                return; // Segmento já removido, ignorar
            }

            allActiveSegments.push(layer);
            ...
            // Obter nós inicial e final do segmento
            // coords são [lng, lat] (formato GeoJSON)
            const startCoord = coords[0];
            const endCoord = coords[coords.length - 1];
            const startKey = this.formatCoordinateKey(startCoord);
            const endKey = this.formatCoordinateKey(endCoord);

            if (!nodeToSegments.has(startKey)) {
                nodeToSegments.set(startKey, new Set());
            }
            if (!nodeToSegments.has(endKey)) {
                nodeToSegments.set(endKey, new Set());
            }

            nodeToSegments.get(startKey).add(layer);
            nodeToSegments.get(endKey).add(layer);
            segmentToNodes.set(layer, [startKey, endKey]);
        });

        if (allActiveSegments.length === 0) {
            return; // Nenhum segmento ativo
        }

        // Determinar ponto de partida (depot ou primeiro segmento)
        let startNodeKey = null;
        
        // Tentar usar o depot se disponível
        const mapManager = this.mapManager;
        if (mapManager && mapManager.getSelectedDepotId) {
            const depotNodeId = mapManager.getSelectedDepotId();
            if (depotNodeId && window.app && window.app.nodeIdToCoordinateMap) {
                const nodeIdToCoordinateMap = window.app.nodeIdToCoordinateMap;
                const depotCoord = nodeIdToCoordinateMap.get(depotNodeId);
                if (depotCoord) {
                    const depotKey = this.formatCoordinateKey(depotCoord);
                    if (nodeToSegments.has(depotKey)) {
                        startNodeKey = depotKey;
                        console.log('Usando depot como ponto de partida:', depotKey);
                    } else {
                        // Depot não está exatamente em um nó, encontrar o mais próximo
                        let minDist = Infinity;
                        for (const nodeKey of nodeToSegments.keys()) {
                            const [lng, lat] = nodeKey.split(',').map(parseFloat);
                            const dx = lng - depotCoord[0];
                            const dy = lat - depotCoord[1];
                        ...
                        if (startNodeKey) {
                            console.log('Depot não encontrado exatamente, usando nó mais próximo:', startNodeKey);
                        }
                    }
                }
            }
        ...
        // Se não encontrou depot, usar o primeiro nó do primeiro segmento ativo
        if (!startNodeKey && allActiveSegments.length > 0) {
            const firstSegment = allActiveSegments[0];
            const firstCoords = firstSegment.feature.geometry.coordinates;
            if (firstCoords && firstCoords.length > 0) {
                startNodeKey = this.formatCoordinateKey(firstCoords[0]);
                console.log('Usando primeiro segmento como ponto de partida:', startNodeKey);
            }
        }

        if (!startNodeKey || !nodeToSegments.has(startNodeKey)) {
            console.warn('Não foi possível determinar ponto de partida para Flood Fill');
            return;
        }

        // Flood Fill: encontrar todos os segmentos alcançáveis a partir do ponto de partida
        ...
        while (queue.length > 0) {
            ...
            const segmentsAtNode = nodeToSegments.get(currentNode);
            if (!segmentsAtNode) {
                continue;
            }

            for (const segment of segmentsAtNode) {
                // Apenas considerar segmentos ainda ativos
                if (segment.__trimState === false) {
                    continue;
                }

                reachableSegments.add(segment);

                // Adicionar o nó oposto do segmento à fila
                const segmentNodes = segmentToNodes.get(segment);
                if (segmentNodes) {
                    for (const nodeKey of segmentNodes) {
                        if (nodeKey !== currentNode && !visitedNodes.has(nodeKey)) {
                            queue.push(nodeKey);
                        }
                    }
                }
        ...
        // Remover segmentos que não são alcançáveis
        let removedCount = 0;
        const totalActive = allActiveSegments.length;
        const totalReachable = reachableSegments.size;
        
        console.log(`Flood Fill: ${totalReachable} de ${totalActive} segmentos alcançáveis a partir de ${startNodeKey}`);
        
        this.geoJsonLayer.eachLayer((layer) => {
            if (!layer || !layer.feature || layer.feature.geometry.type !== 'LineString') {
                return;
            }
            // Apenas processar segmentos que estão marcados como required (ainda ativos)
            if (layer.__trimState !== true) {
                return;
            }

            if (!reachableSegments.has(layer)) {
                // Segmento não alcançável, remover
                layer.__trimState = false;
                if (layer.__trimOriginalRequired === undefined) {
                    layer.__trimOriginalRequired = false;
                }
                this.applyTrimPreviewVisual(layer);
                removedCount++;
            }
        ...
        if (removedCount > 0) {
            console.log(`✅ Removidos automaticamente ${removedCount} segmentos desconectados.`);
        } else if (totalActive !== totalReachable) {
            console.warn(`⚠️ Inconsistência: ${totalActive} ativos mas ${totalReachable} alcançáveis`);
        }
    }

    disableTrimModeInternal(options = {}) {
        if (!this.geoJsonLayer) {
            this.trimModeActive = false;
            return;
        }

        const shouldReset = options.resetStyles === true;

        this.geoJsonLayer.eachLayer((layer) => {
            if (shouldReset && this.geoJsonLayer.resetStyle) {
                this.geoJsonLayer.resetStyle(layer);
            }
            if (layer.__trimClickHandler) {
                layer.off('click', layer.__trimClickHandler);
                delete layer.__trimClickHandler;
            }
            if (layer.getTooltip && layer.getTooltip()) {
                layer.unbindTooltip();
            }
            // Reabilitar popup quando Trim Mode é desativado
            ...
            delete layer.__trimState;
            delete layer.__trimOriginalRequired;
        });

        this.hideTrimNodeMarkers();
        
        // Reabilitar interação com polígonos quando Trim Mode é desativado
        if (this.areaManager && typeof this.areaManager.enablePolygonInteraction === 'function') {
            this.areaManager.enablePolygonInteraction();
        }
    ...
    removeDirectionLayer() {
        if (this.directionLayer) {
            this.mapManager.getMap().removeLayer(this.directionLayer);
            this.directionLayer = null;
        }
    }

    renderDirectionArrows() {
        if (!this.geoJsonLayer || typeof L === 'undefined' || typeof L.polylineDecorator !== 'function') {
            return;
        }
        ...
        this.geoJsonLayer.eachLayer(layer => {
            ...
            // Handle potential nested arrays (MultiLineString)
            ...
            const isOneWay = this.isOneWayRoad(props && props.oneway);
            const arrowColor = this.getArrowColorForFeature(props);
            const basePattern = this.createArrowPattern(arrowColor);

            decorators.push(L.polylineDecorator(latlngs, { patterns: [basePattern] }));

            if (!isOneWay) {
                const reversed = latlngs.slice().reverse();
                decorators.push(L.polylineDecorator(reversed, { patterns: [basePattern] }));
            }
        });

        if (decorators.length > 0) {
            this.directionLayer = L.layerGroup(decorators).addTo(this.mapManager.getMap());
        }
    }

    createArrowPattern(color) {
        return {
            offset: 20,
            repeat: 70,
    ...
    isOneWayRoad(onewayValue) {
        if (!onewayValue) return false;
        const normalized = String(onewayValue).toLowerCase();
        return normalized === 'yes' || normalized === '1' || normalized === 'true';
    }

    getArrowColorForFeature(props) {
        if (!props) return '#1e88e5';
        if (props.isOutsideBoundary) {
            return '#9e9e9e';
        }
        if (props.isRouteRequired === false) {
            return '#9e9e9e';
        }
        if (props.isCovered) {
            return '#fb8c00';
        }
        if (this.isOneWayRoad(props.oneway)) {
            return '#d84315';
        }
        return '#1e88e5';
    }
    ...
    showTrimNodeMarkers() {
        if (!this.mapManager || !this.trimNodeStats || this.trimNodeStats.size === 0) {
            return;
        }
        if (this.trimNodeLayer) {
            this.mapManager.getMap().removeLayer(this.trimNodeLayer);
            this.trimNodeLayer = null;
        }

        const markers = [];
        this.trimNodeStats.forEach((info) => {
            if (!info || !info.coord || info.count < 2) {
                return;
            }
            const lat = info.coord[1];
            const lng = info.coord[0];
        ...
        if (markers.length > 0) {
            this.trimNodeLayer = L.layerGroup(markers).addTo(this.mapManager.getMap());
        }
    }
